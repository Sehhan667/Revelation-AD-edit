...
--------------------------------------------------------------------------------
    ...
    - 穿透式 DDA 增量步进判定命中：
      ...
      + 穿透不挡光（HitLightShpere）；普通固体才命中停止，其中形状块
      （155-294，楼梯/门/栅栏等，IsHitBlock）先做子盒
      求交，穿过子盒空隙则继续步进
...
// （"整个世界全黑"根因）。：无 binding，靠 shaders.properties 的
...
vec4 IrcTraceVoxel(ivec3 c, ivec3 cDi, int cascade, float cellSize) {
    ...
    // ：sampleHemisphere = hasVoxel.x==1 && hasCurrVoxel==0
    //（恰 1 空邻居 && 当前体素非普通方块，用 abs(ID)>240 判"普通"）。
    ...
    for (int s = 0; s < VOXEL_IRC_SPP; ++s) {
        // 随机方向 + 对应 PDF（归一化系数：半球 2·dot、全方向 1.6）
        vec3 dir;
        float pdf;
        if (sampleHemisphere) {
        ...
        for (int i = 0; i < VOXEL_IRC_TRACE_DISTANCE; ++i) {
            ...
            //（用户发现"洞穴亮度受阳光反弹控制"的根因）。hitWorldPos = camrel。
        ...
        if (!hitSolid) {
            // [2026-08-17] 恢复出界天光（8/10 设计）：VoxelSkyColor = LUT × 基准换算 ×
            // 地平线衰减 × 漏光门控。洞穴 hitSkylight≈0 → 无天光（hitSkylight 来自
            // 起点体素 voxelData.w，L228 已解包，与命中路径同口径）。
            ...
            if (dir.y > 0.0 && VoxelSkyLeakGate(hitSkylight) > 0.0)
                exposure += 1.0;
        }
...
void IrcInject(ivec3 c, ivec3 cDi, int cascade, float cellSize) {
    ...
    vec3 skySeed = VoxelSkyColor(vec3(0.0, 1.0, 0.0), seedSkylight)
                 * VOXEL_IRC_EDGE_SEED * step(0.15, seedSkylight);
    vec3 pRC = pValid ? FetchPrevRadianceC(prevC, cascade) : (sld ? skySeed : nRC);
...
void main() {
    ...
    // 时间混合权重：IRC 随机采样靠时域累积降噪（0.99=每帧接受 1% 新值）
    float bw = 1.0 - (1.0 - VOXEL_GI_BLEND) * saturate(frameTime / 0.01666667);
    ...
    for (int vi = ia; vi < ib; ++vi) {
        ...
        // 随机样本闪烁；空气格保持 0（不写入无用值）。
        float seedSkylight = VoxelUnpack2xU8Y(vd.w);
        vec3 skySeed = VoxelSkyColor(vec3(0.0, 1.0, 0.0), seedSkylight)
                     * VOXEL_IRC_EDGE_SEED * step(0.15, seedSkylight);
        vec3 pRC = pValid ? FetchPrevRadiance(prevC) : (sld ? skySeed : nRC);
