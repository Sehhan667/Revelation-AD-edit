...
// 计算彩色阴影（SimpleShadow 逻辑：玻璃等半透明物体会给反弹光线染色）。
...
// 无显式 binding：Iris 按 properties 的 image.<name> = <samplerName> 自动绑定（）。
...
void main() {
        ...
        if (v_isVoxel > 0.5) {
            ...
            // w = Pack2xU8(texRes, skylight)（高 8 位纹理分辨率
