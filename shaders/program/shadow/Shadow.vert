...
// 传递给 GS 的体素化数据（编码语义gbuffers Terrain.vert，坐标系必须与查询端一致）
...
void main() {
    ...
    // 体素化输出（gbuffers Terrain.vert L80-109：世界对齐网格，GS 按质心平铺进阴影左条带）
    #ifdef ENABLE_VOXELIZATION
        ...
        g_notInVoxel = 0.0;
        // PT_FULLBLOCK_DETECTION：未列入 block.properties 的
        ...
        // 发光(20-31)/形状(155-294)/透明等已列出的材料不受影响（只查 g_voxelID<=1）。
        g_posInvalid = 0.0;
        if (materialID == 1u) {
        ...
        // [FIX 2026-08-19 偏移量对齐] 原 0.001 偏移过小（面心向内 ~0.0005 格），
        // 对薄片/不完整方块（地毯/雪层/活板门/楼梯/栅栏）质心距格边界余量不足——
        // 浮点误差 + GS 质心平均（3 顶点均值）后偏移被进一步稀释 → floor() 仍可能
        // 跳格 → 体素写错位 → 形状块子盒空隙漏光。提升至 0.015625（=1/64，
        // 面心向内 ~0.0078 格，
        // 仍远小于半格（0.5）→ 不影响正确 cell，但给薄片/边界面足够余量。
        // 方向语义：toCenter 指向实际形状中心（非固定 +,+,+ 角），薄片
