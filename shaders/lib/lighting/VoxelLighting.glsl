...
// HitLightShpere × BLOCKLIGHT_BRIGHTNESS=0.1（脉冲 0~0.1），
...
#define VOXEL_IRC_EDGE_SEED 0.35        // 0-1 尺度（× skyMapTex 天顶辐射；白天 ≈ 0.1 级注入，等效合理环境光）

// [2026-08-18] 不完整方块挡光模式：体素化把楼梯/台阶/栅栏等形状块写为整格固体，
// 射线却用精确子盒求交——子盒空隙（楼梯缺角/栅栏缝）光线穿过 → 漏光（用户实测
// "所有不完整方块都漏、边缘缝隙状漏光"）。1=子盒求交失败回退整格命中（形状块
// 完全挡光不漏，视觉干净）；0=精确子盒（物理正确，楼梯缺口可透光但会漏光）。
#define VOXEL_SHAPE_SOLID 1 // [0 1] 不完整方块挡光模式（1=整格挡光不漏，0=精确子盒）

// [2026-08-18] 网格边缘环境光过渡带（格数）：新方块进入 64³ 范围时，
...
// NOLIGHT 兜底（NOLIGHT_BRIGHTNESS 思路）：命中固体/出界外的闭塞处底光，
// 洞穴深处不黑死。默认 7e-6 是物理尺度（白天物理辐照度 ~300），
// 本项目 0-1 尺度（skyMapTex 归一化后白天 ~0.1 级注入），等效底光取 0.0005（可调）。
...
// 追踪端阳光弹射强度（sunLight = colorShadowlight，无额外小系数；阴影贴图判定直射）。
